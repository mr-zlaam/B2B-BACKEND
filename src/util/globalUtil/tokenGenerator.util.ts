...
export default {
  generateAccessToken: (payload: IPAYLOAD, res: Response): string | Response => {
    try {
      const token = jwt.sign({ uid: payload.uid, OTP_TOKEN_VERSION: payload.OTP_TOKEN_VERSION, role: payload.role }, envConfig.JWT_SECRET, {
  ...
  generateRefreshToken: (payload: IPAYLOAD, res: Response): string | Response => {
    try {
      const token = jwt.sign({ uid: payload.uid, OTP_TOKEN_VERSION: payload.OTP_TOKEN_VERSION, isExpired: false }, envConfig.JWT_SECRET, {
