...
export function canAccessNextLevel(
  ...
  const millisecondsPassed: number = currentDate.getTime() - unlockedDate.getTime();
  ...
  const isRetentionPeriodValid: boolean = monthsPassed >= retentionPeriodAchievedByUser;
  ...
  const didUserFulfillRetentionPeriodRequirement: boolean = currentDate >= targetDate;
  const didUserFulfillKpiPointsRequirement: boolean = kpiPointsAchievedByUser >= Math.min(requiredKpiPoints, 10);

  if (!didUserFulfillRetentionPeriodRequirement) {
    logger.warn("User did not fulfill the retention period requirement: Retention period is not enough", {
      targetDate: `${targetDate.toLocaleDateString()} - ${targetDate.toLocaleTimeString()}`
    });
    throwError(reshttp.badRequestCode, "User did not fulfill the retention period requirement: Retention period is not enough");
  }

  if (!didUserFulfillKpiPointsRequirement) {
    logger.warn("User did not fulfill the kpi points requirement: KPI points are not enough");
    throwError(reshttp.badRequestCode, `User did not fulfill the kpi points requirement: required kpi points are ${requiredKpiPoints}`);
  }
